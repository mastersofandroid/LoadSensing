...
public class ImatgeXarxaSensors extends Activity {
	...
	class TouchView extends View  {
		...
		public TouchView(Context context) {
			...
			// COGER DE URL			
			overlay = BitmapFactory.decodeResource(getResources(), R.drawable.sensor).copy(Config.ARGB_8888, true);
		}
		...
		public void onDraw(Canvas canvas) {
			...
			Log.i(DEB_TAG, "Xarxa que hem triat anteriorment: " + IdXarxaParam);
			...
			try 
			{
				...
				for (int i = 0; i < llistaSensorsArray.length(); i++) {				
					...
					canvas.drawBitmap(overlay, Integer.parseInt(coordx),  Integer.parseInt(coordy), null);
			...
			switch (action & MotionEvent.ACTION_MASK) 
			{
				case MotionEvent.ACTION_DOWN: {
					for (int j=0; j<listaSensors.size(); j++)
					{
						...
						Log.i(DEB_TAG,"overlay.getHeight() : " + overlay.getHeight());
						Log.i(DEB_TAG,"overlay.getWidth() : " + overlay.getWidth());
						
						if (x >= (coordenadaxsensor-overlay.getWidth()) && x < (coordenadaxsensor + overlay.getWidth())
						            && y >= (coordenadaysensor- overlay.getHeight()) && y < (coordenadaysensor + overlay.getHeight())) 
						{
							Log.i(DEB_TAG, "Click dentro");
							Intent intent = new Intent();
							intent.setClass(getApplicationContext(),
									SingleSensorActivity.class);
							intent.putExtra("idsensorselected", sensor.get("idsensor"));
